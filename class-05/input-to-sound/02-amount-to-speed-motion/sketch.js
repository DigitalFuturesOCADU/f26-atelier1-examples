...
// Here the phone's angle sets it. Flat is slow. Standing upright is fast.
// It is Class 2 · 02 Tilt to Scale, with a sound in place of the image.
// The touch and sound versions change the same loop in the same way.
...
// From Class 2 · 02 Tilt to Scale. rotationX is 0 when the phone lies flat
// and about 90 when it stands upright.

let flatAngle = 0;     // degrees that count as 0
let uprightAngle = 90; // degrees that count as 1

let findings = ['The angle holds the speed. Lay the phone down and it stays slow.'];
...
// how far the phone is tipped up, from 0 to 1. -1 if there are no motion readings
function readAmount() {
  if (!window.sensorsEnabled || !motionArrived) {
    return -1;
  }
  return constrain(map(rotationX, flatAngle, uprightAngle, 0, 1), 0, 1);
}

function inputReading() {
  return 'tilt: ' + round(rotationX) + '°';
}
...
function inputStatus() {
  ...
  return 'lay it flat to slow it, stand it up to speed it up';
}

// the tilt needs nothing extra on screen. the bar up the left edge is the amount.
function drawInput() {
}
